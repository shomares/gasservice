...
public abstract class DaoGasolinerasMongoDB implements IDaoGasolinera {
	...
	public void getAllPlaces() {
		...
		for (Place pl : places.getPlace()) {
			...
			Document place = new Document("id", pl.getPlaceId()).append("brad", pl.getBrad())
					.append("category", pl.getCategory()).append("name", pl.getName()).append("creId", pl.getCreId())
					.append("expires", cl.getTime())
					.append("loc", array);
	...
	@Override
	public List<GasolineraDTO> findGasolineras(Float x, Float y) {
		// TODO Auto-generated method stub
		int i=0, j=0;
		Calendar cl = Calendar.getInstance();
		Date now = cl.getTime();
	...
	public void getAllPrecio() {
		...
		for (Precio.Place place : precio.getPlace()) {
			...
			for (GasPrice gasPrice : place.getGasPrice())
				values.add(new Document().append("fecha", gasPrice.getUpdateTime()).append("Tipo", gasPrice.getType())
						.append("Valor", gasPrice.getValue()));
	...
	public List<PrecioDTO> getPrecio(Integer idGasolinera) {
		Calendar cl = Calendar.getInstance();
		Date now = cl.getTime();
		int i = 0, j = 0;
